...
| Storage | device throughput, IOPS, latency, queue depth와 checkpoint |
...
| `GET /api/runs/<run_id>/storage.json` | device I/O와 checkpoint metric |

현재 run은 `rl-2051`, `pt-1042`, `pt-1041`, `pt-1038`입니다.

## Metric Sources

Dashboard vocabulary는 `sft-lab`의 `profiling/config/metrics.json`을 기준으로 합니다.
